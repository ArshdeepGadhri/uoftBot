...
public class courseNameCommand extends Command {
    EventWaiter waiter;
    public courseNameCommand(EventWaiter waiter){
        this.waiter = waiter;
        this.name = "n";
        this.aliases = new String[]{"name"};
        this.cooldown = 5;
        this.help = "Returns Course data by name";
    ...
    protected void execute(CommandEvent commandEvent) {
        if (commandEvent.getArgs().length()==0){
            commandEvent.reply("Missing args, you need to provide me with a course-name");
            return;
        }
        String courseCode = commandEvent.getArgs().replace(" ","");
            ...
            waiter.waitForEvent(MessageReactionAddEvent.class,
                    ...
                    event -> {
                        ...
                        try{
                            ...
                            HttpGet req = new HttpGet("https://nikel.ml/api/courses?name="+courseCode.replace(" ","%20")+camp.get());
    ...
    private void pageTwo(EmbedBuilder eb, JsonObject data){
        try {
            ...
            try {
                eb.addField("PreReqs", "```\n" + json.get("prerequisites").getAsString() + "```", true);
            }catch (Exception ignored){}
            try {
                eb.addField("CoReqs","```\n"+json.get("corequisites").getAsString()+"```",true);
            }catch (Exception ignored){}

            try{
                eb.addField("Recommended Prep","```\n"+json.get("recommended_preparation").getAsString()+"```",true);
            }catch (Exception ignored){}
            try {

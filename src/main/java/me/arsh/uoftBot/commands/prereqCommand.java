...
public class prereqCommand extends Command {
    EventWaiter waiter;
    public prereqCommand(EventWaiter waiter){
        this.waiter = waiter;
        this.name = "prereq";
        this.aliases = new String[]{"p","prerequisites"};
        this.cooldown = 5;
    ...
    protected void execute(CommandEvent commandEvent) {
        if (commandEvent.getArgs().length()==0){
            commandEvent.reply("Missing args, you need to provide me with prerequisites");
            return;
        }
        String courseCode = commandEvent.getArgs().replace(" ","&prerequisites=").replace(",","");
            ...
            waiter.waitForEvent(MessageReactionAddEvent.class,
                    ...
                    event -> {
                        ...
                        try{
                            ...
                            HttpGet req = new HttpGet("https://nikel.ml/api/courses?prerequisites="+courseCode+camp.get()+"&limit=20");
                            ...
                            for (int i =0; i < jsonO.get("response").getAsJsonArray().size(); i++){
                                co += "- "+jsonO.get("response").getAsJsonArray().get(i).getAsJsonObject().get("code").getAsString() + " | "+jsonO.get("response").getAsJsonArray().get(i).getAsJsonObject().get("name").getAsString()+"\n\n";
                            }
                            co += "```";
                            eb.addField("Displaying First Course Found, Others listed below.",co,false);
                            pageZero(eb,jsonO);
